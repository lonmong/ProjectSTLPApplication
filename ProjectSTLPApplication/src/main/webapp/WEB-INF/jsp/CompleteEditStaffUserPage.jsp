...
<title>แก้ไขข้อมูลเจ้าหน้าที่ศูนย์ให้ความช่วยเหลือฯเสร็จสิ้น</title>
<style>
body {
...
<body>
	...
	<form action="/liststaff" method="post">
		...
		<c:set var="nameperson1" value="${fn:replace(nameperson, '%2F', '/')}" />
		<c:set var="nameperson2"
			value="${fn:replace(nameperson1, '%2C', ',')}" />
		<c:set var="nameperson3" value="${fn:replace(nameperson2, '+', ' ')}" />

		<div class="container">
			<center>
				<font color="#2EDA03"><h1>
						<b>แก้ไขข้อมูลของ "${nameperson3}" เสร็จสิ้น</b>
					</h1></font>
			</center>
